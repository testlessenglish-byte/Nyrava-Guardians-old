...
function HomeHqPage() {
  ...
  const itemCount = progress?.inventory.items.reduce((total, item) => total + item.quantity, 0) ?? 0;
  const certificateCount = progress?.certificates.length ?? 0;
  const trophyCount = progress?.inventory.items.filter((item) => item.id.includes("badge") || item.id.includes("hunter") || item.id.includes("protector") || item.id.includes("keeper")).length ?? 0;
    ...
    <div className="game-viewport hq-viewport fixed inset-0 bg-background font-sans text-slate-100">
      ...
      <div className="absolute bottom-6 left-6 z-40 w-96 rounded-3xl border border-amber-500/40 bg-slate-950/90 p-5 shadow-2xl backdrop-blur-md space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-3"><div className="grid size-12 place-items-center rounded-2xl bg-amber-950 border border-amber-500/50 text-amber-400 font-black text-lg">L{level}</div><div><h3 className="text-base font-black text-white">{guardianName || (es ? "Guardián" : "Guardian")}</h3><p className="text-xs font-bold text-amber-400">{es ? "Rango basado en progreso real" : "Rank based on real progress"}</p></div></div>
          <span className="rounded-full bg-amber-950 border border-amber-500/40 px-3 py-1 text-xs font-black text-amber-300">{xp} XP</span>
        </div>
        <div className="space-y-2"><div className="flex justify-between text-xs font-extrabold text-slate-300"><span>{es ? "Progreso de nivel" : "Level Progress"}</span><span>{xpIntoLevel} / 1000 XP</span></div><div className="h-2.5 w-full rounded-full bg-slate-900 border border-slate-800 overflow-hidden"><div className="h-full bg-amber-400 rounded-full shadow-lg" style={{ width: `${levelPercent}%` }} /></div></div>
        <div className="grid grid-cols-3 gap-2 pt-1">
          <div className="flex flex-col items-center rounded-xl bg-slate-900/80 p-2 border border-slate-800 text-center"><Trophy className="size-5 text-amber-400 mb-1" /><span className="text-[10px] font-black text-slate-200">{trophyCount} {es ? "trofeos" : "trophies"}</span></div>
          <div className="flex flex-col items-center rounded-xl bg-slate-900/80 p-2 border border-slate-800 text-center"><Award className="size-5 text-cyan-400 mb-1" /><span className="text-[10px] font-black text-slate-200">{certificateCount} {es ? "certificados" : "certificates"}</span></div>
          <div className="flex flex-col items-center rounded-xl bg-slate-900/80 p-2 border border-slate-800 text-center"><Package className="size-5 text-emerald-400 mb-1" /><span className="text-[10px] font-black text-slate-200">{itemCount} {es ? "objetos" : "items"}</span></div>
        </div>
      </div>
