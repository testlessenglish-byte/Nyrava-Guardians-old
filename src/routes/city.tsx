...
function DigitalCityPage() {
  ...
  const refreshProgress = () =>
    getProgression()
      .then(setProgress)
      .catch(() => undefined);
  const phishingPassed = Boolean(
    progress?.missions["phishing-defense"]?.completedAt &&
    (progress?.missions["phishing-defense"]?.bestScore ?? 0) >= 75,
  );
  ...
  const inspectTerminal = () => {
    ...
    if (typeof window !== "undefined")
      window.sessionStorage.setItem("nyrava-selected-mission", "phishing-defense");
    if (getPhishingStoryStep() === "INSPECT_TERMINAL")
      advancePhishingStory("INSPECT_TERMINAL", "COMPLETE_ACADEMY_LESSON");
    refreshProgress();
  };
  ...
  return (
    <div className="game-viewport city-viewport fixed inset-0 bg-background">
      <div
        ...
        onPointerMove={(event) => {
          if (dragging.current && !inspecting)
          ...
          <div className="w-full max-w-lg rounded-3xl border border-rose-500/40 bg-slate-950 p-6 space-y-4 shadow-2xl">
            <div className="flex items-center gap-3 text-rose-400">
              ...
              <h2 className="text-xl font-black text-white">
                {es ? "Simulación de mensaje sospechoso" : "Suspicious Message Simulation"}
              </h2>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 text-xs font-mono space-y-2 text-slate-200">
              <p>
                <span className="text-slate-500">From:</span> security-alert@nyrava-login-verify.com
              </p>
              <p>
                <span className="text-slate-500">Subject:</span> URGENT: Account Suspension Notice
              </p>
              <p className="text-slate-300 pt-2 font-sans text-sm font-bold">
                “Your Guardian account will be suspended in 24 hours. Click below to verify your
            ...
            {!phishingPassed ? (
              <div className="rounded-2xl border border-amber-500/40 bg-amber-950/50 p-4 space-y-3">
                <p className="text-sm font-black text-amber-300">
                  {es ? "Entrenamiento requerido" : "Training Required"}
                </p>
                <p className="text-xs text-slate-300">
                  {es
                    ? "Antes de resolver el incidente, completa y aprueba Defensa contra phishing en la Academia."
                    : "Before resolving the incident, complete and pass Phishing Defense in the Academy."}
                </p>
                <Link
                  to="/classroom"
                  onClick={() =>
                ...
                >
                  <Button className="w-full bg-amber-400 text-slate-950 font-black">
                    {es ? "Ir a la Academia" : "Go to Academy"}
                  </Button>
                </Link>
              </div>
            ) : decision === null ? (
              <div className="space-y-2 pt-2">
                <p className="text-xs font-bold text-slate-400">
                  {es ? "¿Qué acción debes tomar?" : "What action should you take?"}
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    onClick={() => setDecision("incorrect")}
                    variant="outline"
                    className="border-rose-500/40 bg-rose-950/40 text-rose-300 font-bold text-xs"
                  >
                    {es ? "Abrir enlace e iniciar sesión" : "Click Link & Sign In"}
                  </Button>
                  <Button
                    onClick={solveIncident}
                    className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs"
                  >
                    {es ? "Reportar y eliminar" : "Report & Delete Message"}
                  </Button>
                </div>
              </div>
            ) : decision === "correct" ? (
              <div className="rounded-2xl border border-emerald-500/40 bg-emerald-950/60 p-4 text-center space-y-3">
                <p className="text-sm font-black text-emerald-400">
                  🎉 {es ? "¡Decisión correcta!" : "Correct Choice!"}
                </p>
                <p className="text-xs text-slate-300">
                  {es
                    ? "Identificaste el dominio falso y reportaste el intento de phishing."
                    : "You identified the fake domain and reported the phishing attempt."}
                </p>
                <Link to="/missions">
                  <Button className="bg-cyan-500 text-slate-950 font-black text-xs">
                    {es ? "Volver con Sarah" : "Return to Sarah"}
                  </Button>
                </Link>
              </div>
            ) : (
              <div className="rounded-2xl border border-rose-500/40 bg-rose-950/60 p-4 text-center space-y-3">
                <p className="text-sm font-black text-rose-400">
                  ⚠️{" "}
                  {es ? "Eso te llevaría al sitio falso" : "That would take you to the fake site"}
                </p>
                <Button
                  onClick={() => setDecision(null)}
                  variant="outline"
                  className="border-slate-700 text-white font-bold text-xs"
                >
                  {es ? "Intentar de nuevo" : "Retry Action"}
                </Button>
              </div>
            )}
            <Button
