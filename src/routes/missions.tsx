...
export const Route = createFileRoute("/missions")({
  ssr: false,
  head: () => ({ meta: [{ title: "Mission Hub — Nyrava Guardians" }, { name: "description", content: "Walkable Guardian mission command room." }] }),
      ...
      {!boardOpen && (
        <>
          <div className="pointer-events-none absolute bottom-6 left-1/2 z-30 -translate-x-1/2 rounded-full border border-cyan-400/30 bg-slate-950/80 px-4 py-2 text-[11px] font-bold text-slate-300 backdrop-blur">
            {es ? "WASD para moverte · arrastra para mirar · habla con Sarah" : "WASD to move · drag to look · talk to Sarah"}
          </div>
          <div className="mobile-game-controls pointer-events-none fixed inset-0 z-40">
            ...
            <div className="game-actions pointer-events-auto"><button type="button" className="game-action" aria-label="Interact" onClick={() => inputManager.triggerInteract()}>Use</button></div>
          </div>
        </>
      )}
      ...
      {boardOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/95 p-5 md:p-8 backdrop-blur-xl">
          <div className="mx-auto max-w-5xl">
            ...
            {selectedTab === "daily" && <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/60 p-8 text-center"><Calendar className="mx-auto size-10 text-cyan-400" /><h3 className="mt-3 text-lg font-black text-white">{es ? "Retos diarios en preparación" : "Daily Challenges In Preparation"}</h3><p className="mt-2 text-sm text-slate-400">{es ? "Se activarán cuando estén conectados al progreso real." : "They will activate when connected to real progression."}</p></div>}
            {selectedTab === "weekly" && <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/60 p-8 text-center"><Award className="mx-auto size-10 text-amber-400" /><h3 className="mt-3 text-lg font-black text-white">{es ? "Operaciones semanales bloqueadas" : "Weekly Operations Locked"}</h3><p className="mt-2 text-sm text-slate-400">{es ? "Se habilitarán cuando exista una operación real." : "They will unlock when a real operation is available."}</p></div>}
          </div>
        </div>
      )}
      <WorldLoading />
