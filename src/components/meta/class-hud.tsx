    ...
    <div className="class-hud pointer-events-none fixed inset-0 z-20 font-sans">
      ...
      <div className="pointer-events-auto absolute left-3 top-3 w-52 space-y-1.5 sm:left-5 sm:top-5 sm:w-64 sm:space-y-2">
        <div className="rounded-xl border border-slate-700/60 bg-slate-950/85 px-2.5 py-1.5 shadow-xl backdrop-blur-md sm:rounded-2xl sm:px-3 sm:py-2">
          <h1 className="text-[10px] font-black uppercase tracking-wider text-white sm:text-xs">{es ? "AULA DE LA ACADEMIA" : "ACADEMY CLASSROOM"}</h1>
          <p className="text-[8px] font-bold tracking-[0.12em] text-slate-400 sm:text-[9px] sm:tracking-[0.15em]">{teacher.name} · {teacher.role}</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-700/70 bg-slate-950/90 p-2.5 text-white shadow-xl backdrop-blur-xl sm:rounded-2xl sm:p-3">
          <p className="text-[8px] font-black uppercase tracking-[0.15em] text-slate-400 sm:text-[9px] sm:tracking-[0.18em]">{es ? "CLASE ACTUAL" : "CURRENT CLASS"}</p>
          <h3 className="mt-0.5 text-xs font-black tracking-tight text-white sm:text-sm">{es ? activeLesson.title.es : activeLesson.title.en}</h3>
          <div className="mt-2 flex items-center justify-between text-xs font-black">
            <span className="text-[10px] text-amber-400 sm:text-[11px]">★ {es ? "Progreso" : "Progress"}</span>
            <span className="text-[10px] text-cyan-300 sm:text-[11px]">{completedStepsCount} / 4</span>
          </div>
      ...
      <div className="pointer-events-auto absolute right-3 top-3 flex items-center gap-2 rounded-xl border border-slate-700/60 bg-slate-950/90 p-1.5 pr-3 shadow-2xl backdrop-blur-md sm:right-5 sm:top-5 sm:gap-3 sm:rounded-2xl sm:p-2 sm:pr-4">
        <div className="grid size-9 place-items-center rounded-lg text-sm font-black text-slate-950 shadow-md sm:size-11 sm:rounded-xl sm:text-base" style={{ background: currentGuardian.color }}>{learnerName.charAt(0).toUpperCase()}</div>
        <div>
          <span className="text-xs font-black text-white sm:text-sm">{learnerName}</span>
          <p className="text-[9px] font-bold text-cyan-300 sm:text-[11px]">{es ? "Guardián" : "Guardian"} · {progress ? `Lvl ${levelFor(progress.xp)}` : "…"}</p>
          <p className="text-[9px] font-black text-amber-400 sm:text-[10px]">★ {progress?.xp ?? 0} XP</p>
        </div>
      </div>
      ...
      {activeInteraction && (
        <div className="pointer-events-auto absolute left-1/2 bottom-20 z-30 -translate-x-1/2 sm:bottom-28">
          <button type="button" onClick={activeInteraction.action} className="rounded-full border border-cyan-400/60 bg-slate-950/95 px-5 py-2 text-[11px] font-black uppercase tracking-wider text-cyan-200 shadow-2xl backdrop-blur-xl hover:bg-cyan-950 sm:px-6 sm:py-2.5 sm:text-xs">{es ? activeInteraction.label.es : activeInteraction.label.en}</button>
        </div>
      )}
      ...
      <div className="pointer-events-auto absolute bottom-4 right-3 flex w-[calc(100vw-1.5rem)] max-w-xs flex-col gap-2 rounded-2xl border border-slate-700/60 bg-slate-950/95 p-3 shadow-2xl backdrop-blur-xl sm:bottom-6 sm:right-6 sm:w-[min(92vw,24rem)] sm:gap-3 sm:rounded-3xl sm:p-4">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <div className="grid size-7 place-items-center rounded-lg text-xs font-black text-slate-950 sm:size-9 sm:rounded-xl sm:text-sm" style={{ background: teacher.color }}>{teacher.name.charAt(0)}</div>
          <div>
            <span className="block text-xs font-extrabold text-white">{teacher.name}</span>
            <span className="block text-[9px] font-bold text-cyan-300 sm:text-[10px]">{es ? "Guía Guardián" : "Guardian Guide"}</span>
          </div>
        </div>
        <div className="max-h-32 space-y-1.5 overflow-y-auto pr-1 text-[11px] sm:max-h-40 sm:space-y-2 sm:text-xs">
          {messages.length === 0 && <p className="rounded-xl bg-slate-900/90 p-2.5 text-slate-200 leading-relaxed border border-slate-800 sm:rounded-2xl sm:p-3">{es ? "Acércate a Sarah y presiona E cuando estés listo." : "Walk up to Sarah and press E when you are ready to start."}</p>}
