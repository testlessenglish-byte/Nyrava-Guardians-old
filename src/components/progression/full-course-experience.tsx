...
import { PHISHING_DEFENSE_COURSE } from "@/domain/progression/courses/phishing-defense-course";
...
export function FullViewportCourseExperience({
  ...
  if (activeMission.id === "phishing-defense") {
    return (
      <div
      ...
      >
        <div className="w-full max-w-4xl mx-auto flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
          ...
          <div className="text-right">
            ...
            <h1 className="text-sm font-black text-white">{text(PHISHING_DEFENSE_COURSE.title)}</h1>
          </div>
        </div>

        <CourseEngine
          course={PHISHING_DEFENSE_COURSE}
          locale={locale}
          onCourseComplete={async (result) => {
            const finalScore = result?.score ?? 100;
            if (attemptId) {
              try {
                const answersArray = PHISHING_DEFENSE_COURSE.assessment.questions.map(
